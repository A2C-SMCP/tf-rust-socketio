...
impl RawClient {
    ...
    /// # Example
    /// ```
    /// use tf_rust_socketio::{ClientBuilder, RawClient, Payload};
    /// use serde_json::json;
    ///
    ...
    ///             println!("{:#?}", values);
    ///             // Respond with the specific ack_id
        ...
        D: Into<Payload>,
    {
        ...
        self.socket.ack(&self.nsp, data.into(), None)
    }

    /// Acknowledge a message with a specific ack_id
    #[inline]
    pub fn ack_with_id<D>(&self, ack_id: i32, data: D) -> Result<()>
    where
        D: Into<Payload>,
    {
        self.socket.ack(&self.nsp, data.into(), Some(ack_id))
    }
    ...
    // NOTE (issue #12 follow-up): unlike the async client, this synchronous
    // client keeps the upstream "single reader, strictly serial dispatch"
    // contract — user callbacks run inline on the polling caller's thread. If
    // a long callback is registered, heartbeats and later packets stall here
    // too. The sync side is intentionally out of scope of #12; if the serial
    // contract needs to be relaxed, a per-event worker-thread dispatch (or a
    // documented "no long callbacks" rule) is the follow-up.
...
mod test {
    ...
    fn socket_io_integration() -> Result<()> {
        ...
        let payload = json!({"token": 123});
        #[allow(deprecated)]
        let result = socket.emit("test", Payload::String(payload.to_string(), None));

        assert!(result.is_ok());

        let ack_callback = move |message: Payload, socket: RawClient| {
            let result = socket.emit("test", Payload::Text(vec![json!({"got ack": true})], None));
            assert!(result.is_ok());
    ...
    fn socket_io_on_any_integration() -> Result<()> {
        ...
        let _socket = ClientBuilder::new(url)
            ...
            .on("auth", |payload, _client| {
                if let Payload::Text(payload, _) = payload {
                    println!("{:#?}", payload);
                }
            })
